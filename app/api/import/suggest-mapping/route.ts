...
function sanitizeMapping(headers: string[], mapping: Partial<Mapping>): Mapping {
  const out: any = {
    sku: "",
    name: "",
    stock: "",
  ...
  (Object.keys(out) as MappingKey[]).forEach((k) => {
    const v = (mapping as any)?.[k];
    if (typeof v === "string" && v && headers.includes(v)) out[k] = v;
  });
...
export async function POST(req: Request) {
  ...
  const heur = guessMappingSmart(headers, sampleRows);

  // If no API key configured, return heuristic immediately.
  ...
  const prompt = {
    role: "user" as const,
    content:
      "Sos un asistente que ayuda a mapear columnas de importación de inventario.\n" +
      "Necesito elegir, entre estos encabezados EXACTOS, qué columna corresponde a cada campo: sku, name, stock, cost, price, supplier, category.\n" +
      "Reglas:\n" +
      "- Solo podés devolver valores que EXISTAN en headers. Si no hay buen match, usá \"\".\n" +
      "- Devolvé JSON estricto (sin texto extra, sin markdown) con esta forma: { mapping: {sku,name,stock,cost,price,supplier,category}, confidence: {sku,name,stock,cost,price,supplier,category}, notes: string[] }\n" +
  ...
  const confidence: Record<MappingKey, number> = { ...heur.confidence };
  if (obj?.confidence && typeof obj.confidence === "object") {
    (Object.keys(confidence) as MappingKey[]).forEach((k) => {
  ...
  // If AI failed to give parseable JSON, fall back to heuristics.
  if (!usedAI) {
    return NextResponse.json({
      usedAI: false,
      mapping: heur.mapping,
